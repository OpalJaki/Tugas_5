...
prompt_item: Pesan yang ditampilkan kepada pengguna untuk meminta input item.

- item_label: Digunakan untuk menampilkan tanda " - " di depan setiap item yang dimasukkan.
- list_title: Judul untuk daftar belanja yang akan ditampilkan setelah selesai.
- newline: Karakter newline (\n) untuk memisahkan setiap item di layar.
- done_str: String untuk memeriksa jika input pengguna adalah "done".
...
input_loop:
    ...
    mov ecx, input
```
Di bagian .bss, variabel-variabel yang belum diinisialisasi dideklarasikan dan ruang memori untuk mereka dialokasikan.

- input resb 32: Mengalokasikan 32 byte untuk buffer input yang akan digunakan untuk menyimpan data yang dimasukkan oleh pengguna. Buffer ini mampu menyimpan string dengan panjang maksimum 31 karakter (termasuk karakter terminator \0).

- items resb 32 * 5: Mengalokasikan 160 byte (32 byte untuk setiap item × 5 item maksimal) untuk menyimpan daftar belanja yang dimasukkan oleh pengguna. Setiap item dapat memiliki panjang hingga 32 byte.

- item_count resb 1: Mendeklarasikan variabel item_count yang akan menyimpan jumlah item yang telah dimasukkan oleh pengguna. Ini adalah 1 byte yang digunakan untuk menghitung jumlah item yang dimasukkan.

Pada bagian .text berisi kode yang akan dieksekusi oleh CPU, dimulai dengan titik masuk _start yang merupakan awal dari eksekusi program.

Di bagian _start, program dimulai dengan menginisialisasi item_count ke 0 menggunakan instruksi xor byte [item_count], 0. Hal ini memastikan bahwa tidak ada item yang dimasukkan pada awalnya.
...
Menampilkan Prompt: Program menampilkan prompt untuk meminta input pengguna menggunakan sistem call write. Register eax diatur ke 4 (nomor sistem call untuk write), ebx diatur ke 1 (stdout), dan ecx diatur ke alamat string prompt_item, yang berisi pesan "Enter item (or type 'done' to finish):".

Membaca Input: Program membaca input dari pengguna menggunakan sistem call read dengan register eax diatur ke 3 (nomor sistem call untuk read). Program membaca hingga 32 byte dari input standar (stdin) dan menyimpannya di buffer input.
